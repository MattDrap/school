...
trainOpts.expDir = 'mnist/baseline' ;

% Call training function in MatConvNet
[net,info] = cnn_train(net, imdb, @getBatch, trainOpts) ;

%% The baseline network - testing

% load the network with the lowest validation error for testing
[isolated_error_baseline, ix] = min(info.val.error(1,:))
load([trainOpts.expDir, '/net-epoch-', num2str(ix)]);
net.layers{end}.type = 'softmax'; %softmax loss -> softmax (for testing)

% Continous recognition - baseline network
I = imread('phone_num_04.jpg');
...
imagesc(Simg);
set(gca, 'ytick', 1:10); set(gca, 'yticklabel', imdb.meta.classes); grid on
ylabel('labels'); 
title('response map');
set(gcf, 'name', 'Baseline-network'); 

% In context recognition
Contextdb = load('context_set_1');

%visualize the digit with context
figure(5); 
vl_imarraysc(squeeze(Contextdb.imgs)); axis image; colormap gray
set(gca, 'xtick', 1:28:1000); set(gca, 'ytick', 1:28:1000);
set(gca, 'xcolor', 'r'); set(gca, 'ycolor', 'r'); 
set(gca, 'xticklabel', ''); set(gca, 'yticklabel', ''); 
grid on
title('Context test set'); 

res = vl_simplenn(net, single(Contextdb.imgs), [], [], 'mode', 'test');
scores = squeeze(gather(res(end).x));
[s, labels_] = max(scores,[],1); 
context_error_baseline = sum(labels_ ~= Contextdb.labels) / length(Contextdb.labels) 

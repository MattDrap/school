...
for i=1:n
  ...
  switch l.type
    ...
    case 'pdist'
      %res(i+1).x = vl_nnpdist(res(i).x, l.p, 'noRoot', l.noRoot, 'epsilon', l.epsilon) ;
      res(i+1).x = vl_nnpdist(res(i).x, shiftdim(l.class,-2), l.p, 'noRoot', l.noRoot, 'epsilon', l.epsilon) ;
      
    case 'mixedloss'  %mixed classification and regression (JC)
        res(i+1).x = vl_nnmixedloss(res(i).x, l.labels, l.class_fields, l.target_var_weights) ;
...
%                                  'noRoot', l.noRoot, 'epsilon', l.epsilon) ;
        res(i).dzdx = vl_nnpdist(res(i).x, shiftdim(l.class,-2), l.p, res(i+1).dzdx, ...
            'noRoot', l.noRoot, 'epsilon', l.epsilon) ;

      case 'mixedloss'  %mixed classification and regression (JC)
        res(i).dzdx = vl_nnmixedloss(res(i).x, l.labels, l.class_fields, l.target_var_weights, res(i+1).dzdx) ;

...
im1 = imread('pokemon_03.jpg');
im2 = imread('pokemon_29.jpg');
ims = {im1, im2};
clear im1;
clear im2;
...
K = [k11, 0, k13;
 ...
 Kmt = iK' * iK;
  ...
  angles = zeros(4, 1);
 angles(1) = (vp1(:, 1)' * Kmt * vp1(:, 3)) / (norm(iK*vp1(:, 1)) * norm(iK*vp1(:, 3)));
 angles(2) = (vp2(:, 1)' * Kmt * vp2(:, 3)) / (norm(iK*vp2(:, 1)) * norm(iK*vp2(:, 3)));
 angles(3) = (vp1(:, 2)' * Kmt * vp1(:, 4)) / (norm(iK*vp1(:, 2)) * norm(iK*vp1(:, 4)));
 angles(4) = (vp2(:, 2)' * Kmt * vp2(:, 4)) / (norm(iK*vp2(:, 2)) * norm(iK*vp2(:, 4)));

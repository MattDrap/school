	...
	while (edges > 3 && !q.empty()) {
		...
		if (pred == LEFT_TURN) {
			if (isEar<KERNEL>(Vert, Vert1, Vert2, mesh, he2)) {
				...
				if (qv.find(Vert) != qv.end()) { //FOUND
					auto it = qv.find(Vert);
					qv.erase(it);
				...
				OrientationType pred = Orient(mesh.point(Vert0), mesh.point(Vert), mesh.point(Vert2));
				if (pred == LEFT_TURN) {
					if (isEar<KERNEL>(Vert0, Vert, Vert2, mesh, he1)) {
				...
				pred = Orient(mesh.point(Vert), mesh.point(Vert2), mesh.point(Vert3));
				if (pred == LEFT_TURN) {
					if (isEar<KERNEL>(Vert, Vert2, Vert3, mesh, mesh.next_halfedge_handle(he2))) {
						q.push_back(he1);
						qv.insert(Vert2);
					}
				}
...
bool MakeDelaunayByDiagonalFlipping(typename KERNEL::MeshType &mesh)
{
	...
	while (!q.empty())
	{
		...
		//RATHER TEST ALL 3 Possibilities - CAN'T GET OPTIMAL VERTEX WITH OPENMESH
		do {
